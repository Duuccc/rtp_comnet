...
def simulate_network_loss(packets, loss_rate=0.1):
...
def main():
    ...
    sender = RTPSender(ssrc=12345, group_size=4)
    receiver = RTPReceiver(expected_ssrc=12345, group_size=4)
    ...
    received_packets = simulate_network_loss(sent_packets)
    ...
    ordered_packets = receiver.get_ordered_packets()
    print(f"Final ordered packets: {len(ordered_packets)}")
    
    # Check recovery success
    recovery_rate = len(ordered_packets) / len(sent_packets)
